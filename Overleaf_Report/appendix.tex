...
\begin{table}[H]
\centering
\caption{Upstream route-selection summary for Experiment 2. The masked-adaptation route was rejected because it did not improve downstream three-class QA. The binary weak-label stage uses only \textit{yes}/\textit{no}, so \textit{maybe} is not applicable there.}
\label{tab:bcb_route_summary}
\small
\resizebox{\linewidth}{!}{%
...
\hline
Supervised base route (3-seed mean) & 0.493 & 0.476 & 0.366 & 0.492 & 0.568 \\
MLM-adapted route (pilot downstream check) & 0.480 & 0.453 & 0.333 & 0.444 & 0.582 \\
